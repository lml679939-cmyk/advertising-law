...
|------|--------|--------|------|
| 是非題 | 59 | 0 | **59** |
| 選擇題 | 59 | 0 | **59**（由是非題轉換而來）|
| 填充題 | 0 | 0 | **0**（待建置）|
| **合計** | **118** | **0** | **118** |

> 期末選擇題（59 題）為本次 session 由同一批是非題一對一轉換，考點完全對應。
...
├── index.html              ← 期中（不再大改）
├── final/
│   └── index.html          ← 期末（59 題是非 + 59 題選擇，共 118 題）
├── images/                 ← 共用角色立繪
├── midterm-exam/           ← 期中考 PDF 及匯出 TXT
├── final-exam/             ← 期末考 PDF 及匯出 TXT（已含選擇題）
└── quiz_export/
    └── export_all.py       ← 匯出腳本
```
...
- [x] 建置選擇題（59 題，由是非題一對一轉換，考點完全對應）
- [x] NotebookLM 匯出 TXT 更新（`final-exam/期末題庫完整內容_NotebookLM確認用.txt`，tf=59, mc=59）
...
- **NotebookLM 驗證**：`final-exam/期末題庫完整內容_NotebookLM確認用.txt` 已含 59 題是非＋59 題選擇，可上傳 NotebookLM 繼續驗證
